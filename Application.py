...
def main():
    ...
    ManageDb.insert_n_n_test(Product.instantiated_products, NAME_OF_TABLE[1], NAME_OF_TABLE[0],
                             NAME_OF_TABLE[6], COLUMN[8], COLUMN[5])
    # problème surement lié a l'argument ( recherche des cat dans product) il trouve les stores ou les barnd
    ManageDb.select(SQL_ARGS, NAME_OF_TABLE[6])
